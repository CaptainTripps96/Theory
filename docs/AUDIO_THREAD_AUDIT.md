...
`TracktionPlaybackEngine::createProjectMidiClip()` is the final MIDI materialization step for normal hosted-plugin tracks. It traverses TheorySequencer clips and notes on the control path, not from the audio callback. The audio callback consumes the prepared Tracktion edit.

Native Simple Osc Complex tracks intentionally use a separate prepared path: MIDI clip notes, loop repetitions, pitch slurs, and vibrato offsets are converted on the sync/control side into native scheduled note, slur, and pitch-offset events. The Tracktion MIDI clip for those native tracks is only a lightweight graph/wake surface and does not duplicate note playback. This keeps first-party expression semantics and ordinary no-expression Simple Osc notes on the same prepared native event path.

The prompt's precomputed MIDI playback-buffer requirement is therefore satisfied for MVP by prepared backend data: hosted-plugin tracks use Tracktion sequence materialization, while native Simple Osc tracks use prepared native event vectors ahead of render time.
...
- Native Simple Osc Complex playback uses Tracktion's plugin render callback, but app-owned preparation is kept on the sync/control side: first-party `ValueTree` state is converted to a cached patch during plugin initialisation, note/slur/vibrato intent is converted to scheduled native events, and expression routes are converted to validated, sorted modulation streams with numeric targets before `applyToBuffer()` renders. The render callback does not resize host buffers; if Tracktion ever supplies fewer than the declared stereo channels, the plugin clears the block and returns rather than allocating.
